...
`estimate_lppd()` estimates the posterior predictive density from a sample, in a similar way to the algorithm described above. `compute_lppd()` calculates the full log posterior predictive density analytically. See the [GitHub Issue #4](https://github.com/phac-modelling-hub/immunity-landscape/issues/4) for a discussion of methodology.
...
province_relation <- readr::read_csv(here::here("data", "3610040201-noSymbol.csv"), show_col_types = F) %>%
  filter(location %in% unique(vax_clean$location))  # province GDP data from StatCan
prov_levels <- (province_relation %>% pull(location))  # useful shortcut for defining province factors
prov_values <- log(province_relation %>% pull(Dollars))
...
example2 <- compute_lppd(vax_dataset, prov_values=prov_values, k=ksqexp, l1=1.5, l2=3, b=1)  # run time: <1 second
...
fit_GP <- function(vax_dataset, last_agecurrent=30, prov_values, ndrws=100, k_list=list(ksqexp=ksqexp), l1=NA, l2=NA, b=1) {
  ...
  combinations <- combinations %>% mutate(lppd_sum_estimate = pmap_dbl(list(k, l1, l2, b), ~ estimate_lppd(  # method 1: estimate (l)ppd
    ...
    l2 = ..3,
    b = ..4)))
  combinations <- combinations %>% mutate(lppd_sum_exact = pmap_dbl(list(k, l1, l2, b), ~ compute_lppd(
    vax_dataset = vax_dataset,
    last_agecurrent = last_agecurrent,
...
combinations <- fit_GP(vax_clean, prov_values=prov_values, k_list=list(ksqexp=ksqexp), l1=c(1.5,1.6), l2=c(3), b=c(1))

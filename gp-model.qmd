...
vax_cNICS <- readr::read_csv(here::here("data", "measles_CNICS.csv"), show_col_types = FALSE) %>% filter(!(pt %in% c("SK","YT","NB","CA"))) %>%
  mutate(age_current = current_year - year_birth)
...
prov_values <- extract_province_relation("GDP", vax_dataset=vax_clean)
prov_values <- extract_province_relation("HIDT", vax_dataset=vax_clean)
```

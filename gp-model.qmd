...
#' code for defining province relation (prov_values)
province_relation_data <- readr::read_csv(here::here("data", "3610040201-noSymbol.csv"), show_col_types = F) %>%
  filter(location %in% unique(vax_clean$location)) %>% mutate(value = log(Dollars))  # province GDP data from StatCan (log)
...
example1 <- estimate_lppd(vax_clean, k=ksqexp, l1=1.5, l2=3, b=1)  # run time: ~15 seconds

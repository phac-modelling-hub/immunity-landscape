...
yobs <- yvals[vax_clean %>% filter(age_current %in% c(4,9,13,19,20,21,25,30)) %>% pull(location)]  # Alberta==25.55, BC==25.31 etc as in yvals (log(GDP))  **already scaled by l2
zobs <- vax_clean %>% filter(age_current %in% c(4,9,13,19,20,21,25,30)) %>% pull(value)

#' alternatively uncomment to observe all available data
# xobs <- vax_clean %>% filter(age_current<=last_agecurrent) %>% pull(age_current)
# yobs <- yvals[vax_clean %>% filter(age_current<=last_agecurrent) %>% pull(location)]
# zobs <- vax_clean %>% filter(age_current<=last_agecurrent) %>% pull(value)
...
As before, we can compare performance by adding back in data points that the model has not seen in green:
...
#' x and y are independent variables, with dependent variable z
run_GP <- function(vax_dataset, first_agecurrent=NA, last_agecurrent=NA, yvals=NA, k=NA, k_name=NA, l1=NA, l2=NA, ndrws=50) {
  #' define possible x values
  if (is.na(first_agecurrent)) first_agecurrent <- 1  # first age for the GP model
  if (is.na(last_agecurrent)) last_agecurrent <- 32  #age_pre1970-1  # last age for the GP model 
  xvals <- first_agecurrent:last_agecurrent  # a vector of current ages

  #' define possible y values (yvals is a named vector)
  province_relation <- readr::read_csv(here::here("data", "3610040201-noSymbol.csv"), show_col_types = F) %>%
    filter(location %in% unique(vax_clean$location))  # province GDP data from StatCan
  prov_levels <- (province_relation %>% pull(location))  # useful shortcut for defining province factors
  ...
  if (is.na(l1)) l1 <- 2  # x lengthscale
  if (is.na(l2)) l2 <- 3  # relative lengthscale of x values to y values
  yvals <- yvals*l2  # scale y values to the x lengthscale

  #' compute and plot prior
  prior2D <- compute_prior2D(xvals=xvals, yvals=yvals, k=k, l1=l1, ndrws=ndrws, prov_levels=prov_levels)
  ...
  xobs <- vax_dataset %>% filter(age_current<=last_agecurrent) %>% pull(age_current)
  yobs <- yvals[vax_dataset %>% filter(age_current<=last_agecurrent) %>% pull(location)]  # this includes scaling by l2
  zobs <- vax_dataset %>% filter(age_current<=last_agecurrent) %>% pull(value)
  tibble(xobs, yobs, zobs, logit(zobs)) %>% print()

  #' compute and plot posterior
  post2D <- compute_posterior2D(xvals=xvals, yvals=yvals, xobs=xobs, yobs=yobs, zobs=zobs,
                              k=k, l1=l1, ndrws=ndrws, prov_levels=prov_levels)

...
We use the squared exponential covariance function to begin with, with current age lengthscale, `l1`, and _relative_ lengthscale of current age to province, `l2`. Relation of provinces currently uses GDP from StatCan (https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=3610040201) -- this should be updated when a better method is found. Define the prior ~ Normal(0,cov) as before and draw a sample. 
...
xvals <- first_agecurrent:last_agecurrent  # a vector of current ages (these x_i's are equally spaced) 
province_relation <- readr::read_csv(here::here("data", "3610040201-noSymbol.csv"), show_col_types = F) %>% filter(location %in% unique(vax_clean$location))  # province GDP data from StatCan
...
l1 <- 2  # x lengthscale
l2 <- 2  # relative lengthscale of x values to y values
...
prior2D <- tibble() 
for(i in 1:1){  # 50 realisations from a 2D gaussian process, with both x, y as independent variables
...
prior2D <- prior2D %>% mutate(y_label = names(yvals)[match(y_i, yvals)]) %>% 
  mutate(y_label = factor(y_label, levels=c("Ontario","Quebec","Alberta","British Columbia","Manitoba","Nova Scotia","Newfoundland and Labrador","Prince Edward Island","Northwest Territories","Nunavut")))
...
prior2D %>% ggplot(aes(x=x_i, y=y_i, z=prior2D, group=factor(draw))) + geom_contour()
```
...
yobs <- yvals[vax_clean %>% filter(age_current %in% c(4,9,13,19,20,21,25,30)) %>% pull(location)]  # Alberta==25.55, BC==25.31 etc as in yvals (log(GDP))
...
yobs <- yobs/l2  # scale y data by relative lengthscale l2

tibble(xobs, yobs, zobs, logit(zobs))
```

## Posterior
As in the 1D case, the posterior is uniquely defined by its posterior mean and posterior covariance matrix. First, calculate individual covariance matrices to determine the pairwise 'relationships' between unobserved `{xvals, yvals}` and observed `(xobs, yobs)`. Then calculate the posterior covariance matrix as `kuu - kuo%*%solve(koo)%*%kou`.
In the 1D case, our unobserved was `xvals`, now it is the concatenated vector grid `{xvals, yvals}` of size 320x320 (32 ages in each of 10 provinces).
```{r}
#| label: posterior_2D
...
kuo <- generate_2Dksqexp_covmat(xygrid,xyobs,fn=ksqexp,l=l1)  # 'relationship' pairwise between unobserved and observed
kou <- generate_2Dksqexp_covmat(xyobs,xygrid,fn=ksqexp,l=l1)  # 'relationship' pairwise between observed and unobserved
koo <- generate_2Dksqexp_covmat(xyobs,xyobs,fn=ksqexp,l=l1) + 1e-6*diag(nrow(xyobs))  # protect against non-invertibleness
kuu <- generate_2Dksqexp_covmat(xygrid,xygrid,fn=ksqexp,l=l1)
...
post2D <- tibble()
for(i in 1:1) {  # take 50 posterior draws
...
post2D <- post2D %>% mutate(post2D_constrained=invlogit(post2D)) %>%  # add column to transform back to [0,1]
  mutate(y_label = as.factor(names(yvals)[match(y_i, yvals)])) %>%  # add column to return province names
  select(draw,x_i,y_i,y_label,post2D,post2D_constrained)  # reorder columns

#' plot posterior sample 
yobs_factors <- factor(names(yobs), levels(factor(post2D$y_label)))
post2D %>% ggplot() + geom_point(aes(x=x_i,y=post2D,group=factor(draw)), alpha=0.3) +
...
# post2D %>% ggplot() + geom_point(aes(x=x_i,y=post2D_constrained,group=factor(draw)), alpha=0.3) + scale_x_continuous(breaks=xvals,name="current age x_i") + geom_line(aes(x=x_i,y=post2D_constrained,group=factor(draw)), alpha=0.2) + geom_point(data=tibble(x=xobs, y_label=names(yobs), y=zobs), aes(x=x,y=y), col="red") + facet_wrap(~ y_label)

...
From a quick by-eye model fitting, current best GP model parameter values are approximately `l1=1.5, l2=3, b=0.75` for `k=ksqexp`. A more formal fit could also be done, using for example take-one-out cross-validation with log posterior predictive density (lppd) as the 'error' metric (\*\*functionality not yet added).
...
## Formal model fitting: take-out-one cross validation

We aim to find the optimal choice of model parameters (`k`, `l1`, `l2`, `b`), using take-one-out cross-validation. An example algorithm is as follows:
...
lppd(vax_clean, k=ksqexp, l1=1.5, l2=3, b=1)  # run time: ~65 seconds


#' Fit the full 2D Gaussian Process model -- identify optimal combination of k, l1, l2, b. Outputs a 
#' tibble detailing each set of model parameters and a goodness-of-fit metric (sum(lppd)) using take-one-out cross validation. 
...
fit_GP <- function(vax_dataset, last_agecurrent=30, ndrws=50, k_list=list(ksqexp=ksqexp), l1=NA, l2=NA, b=1) {
  ...
  combinations <- combinations %>% mutate(lppd_sum = pmap_dbl(list(k, l1, l2, b), ~ lppd(

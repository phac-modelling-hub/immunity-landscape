...
covmat <- expand_grid(x1=xvals,x2=xvals) %>% 
  mutate(k=ksqexp(abs(x1-x2),l)) %>%  # evaluation of cov function pairwise for ages x1_i and x2_i (i=1,..,n)
  #mutate(k=klin(x1,x2,1,2)) %>% 
...
# generate_klin_covmat <- function(x1,x2,sigma_int,sigma_slope) {
#   expand_grid(x1=x1,x2=x2) %>% mutate(k=klin(x1,x2,sigma_int,sigma_slope)) %>%  # pairwise evaluation of cov function
#     pull(k) %>% matrix(nrow=length(x1),byrow=T)
# }
# kuo <- generate_klin_covmat(xvals,xobs,1,2)  # 'relationship' pairwise between unobserved and observed ages
# kou <- generate_klin_covmat(xobs,xvals,1,2)  # 'relationship' pairwise between  observed and unobserved ages
# koo <- generate_klin_covmat(xobs,xobs,1,2) + 1e-3*diag(length(xobs))  # protect against non-invertibleness
# kuu <- generate_klin_covmat(xvals,xvals,1,2)

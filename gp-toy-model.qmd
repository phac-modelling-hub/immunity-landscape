...
LH: I'll add more on Monday but just to note that we aren't using getting any info. The covariance matrix is calculated using only the independent variable (age value, e.g. 5 and 6) and does not know the coverage at each age point. It's purely a measure of how related the items on the x-scale are to each other (as I understand).

IP: Can you pls point me to some reading material and/or your notes where the posterior covariance matrix is derived? LH: Nice visual website here https://distill.pub/2019/visual-exploration-gaussian-processes/ -- in the section on 'Marginalising and Conditioning', the conditioning X\|Y expression is described. \*\*I will update with a better resource.

...
LH: Nice visual website here https://distill.pub/2019/visual-exploration-gaussian-processes/ -- in the section on 'Marginalising and Conditioning', the conditioning X|Y expression is described. **I will update with a better resource.
```{r}
#| label: posterior
...
for(i in 1:1){  # a single realisation from a 2D gaussian process, with both x, y axes independent variables - one draw from the prior
  gpdraw_df <- bind_rows(gpdraw_df, tibble(draw=i, x=xydf$x, y=xydf$y, z=rmnorm(1, rep(0,npts^2), covmat + 1e-6*diag(npts^2))))
}

gpdraw_df %>% ggplot(aes(x=x, y=y, z=z, group=factor(draw))) + geom_contour()
```

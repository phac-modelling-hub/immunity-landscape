make_fig_england_accuracy <- function(accuracy, error_tolerance, data_split, fit_hp, accuracy_branch){
  # Prep data for plotting and summarizing
branch_names <- unique(stringr::str_extract(names(accuracy), "accuracy(_supp)?_[[:alnum:]]+"))
...
panel_B <- make_england_posterior_plot(
  ...
  panel_A / panel_B + patchwork::plot_layout(heights = c(1, 2))
}
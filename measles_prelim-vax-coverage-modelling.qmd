...
vax_clean1 <- vax_deduped %>% ungroup() %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>%
  ...
  bind_rows(vax_aNICS_full %>% filter(age==18)) %>%  # attach one adult estimate from aNICS for each province (e.g. age 18)
...
vax_clean2 <- vax_clean2_noadults %>% bind_rows(vax_aNICS_full %>% filter(age==18)) %>%  # attach one adult estimate from aNICS for each province (e.g. age 18)

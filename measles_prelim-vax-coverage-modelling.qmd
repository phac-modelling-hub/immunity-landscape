...
We fill in year_birth where it is missing and append adults estimates.
```{r}
#| label: prep_to_export_vax
...
```
```{r}
#| label: prep_to_export_vax_deduped
...
vax_new2 <- vax_new %>%
  ...
  # choose the most recent estimate (by current age) using year reported
...
```
```{r}
#| label: prep_to_export_vax_dedupe

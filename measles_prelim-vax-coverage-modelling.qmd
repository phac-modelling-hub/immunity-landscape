...
vax_clean <- vax_aged %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>% bind_rows(vax_aNICS_full) # attach adult estimates from aNICS

# export
vax_clean %>% bind_rows(vax_aNICS_full) %>%  # attach adult estimates from aNICS
  readr::write_csv(here::here("data", "measles_vax-coverage-data-clean.csv"))
```
...
vax_clean1 <- vax_deduped %>% ungroup() %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>%
  ...
  bind_rows(vax_aNICS_full) %>% group_by(pt, location, age_current) %>% filter(value == max(value)) # attach adult estimates from aNICS and keep largest estimate if there are duplicates (as aNICS likely underreports?)
...
vax_clean2 <- vax_clean2_noadults %>%
    bind_rows(vax_aNICS_full) %>% group_by(pt, location, age_current) %>% filter(value == max(value)) # attach adult estimates from aNICS and keep largest estimate if there are duplicates (as aNICS likely underreports?, and for estimates with 1+ and 2 dose we wish to keep the larger)

...
vax_clean <- vax_aged %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>% 
  bind_rows(vax_aNICS_full)  # attach all adult estimate from aNICS

# export
vax_clean %>% readr::write_csv(here::here("data", "measles_vax-coverage-data-clean.csv"))
```
...
vv_add1dose <- vv2 |>
  ...
  mutate(`1` = `1+` - `2+`, is_incompatible = `1` < 0)  ##LH: this doesn't work for Alberta currently (need '2').
...
vax_deduped <- vv2 |>
  ...
  # for compatible estimates, attach inferred 1-dose value  ##LH: year_report/ age are not carried over.
...
```
```{r}
#| label: export_vax_deduped

# fill in year_birth where it is missing and assume 2+ = 2 doses for now (as same vaccine efficacy used currently)
vax_clean1 <- vax_deduped %>% ungroup() %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>%
  mutate(n_doses = ifelse(n_doses=="2+", "2", n_doses)) %>% select(-`2`) %>%
  bind_rows(vax_aNICS_full %>% filter(age_current==25)) %>%  # attach one adult estimate from aNICS for each province (e.g. age 25)
  group_by(pt, location, age_current) %>% filter(value == max(value)) # keep largest estimate if there are duplicates (as aNICS likely underreports?)

# export -- vax_clean1 is the cleaned data set for take 1
vax_clean1 %>% readr::write_csv(here::here("data", "measles_vax-coverage-data-clean1.csv"))
```
...
LH: In many cases (e.g. Alberta, Manitoba, BC), the exactly 1-dose estimates are usually because the individual is not old enough to have received a second dose at the time of reporting. Many of those individuals likely later received their second dose. Since we are transforming to current age, we could consider assuming 1 dose becomes 1+ if current age is now greater than the age at which second dose is given. I think this might lead to more meaningful interpolation/extrapolation for exactly 1 dose?
```{r}
...
```
```{r}
#| label: export_vax_dedupe

# fill in year_birth where it is missing and assume 2+ = 2 doses for now (as same vaccine efficacy used currently)
vax_clean2_noadults <- vax_dedupe %>% ungroup() %>% mutate(year_birth = ifelse(is.na(year_birth) & !is.na(year_report) & !is.na(age), year_report - age, year_birth)) %>%
  mutate(n_doses = ifelse(n_doses=="2+", "2", n_doses)) %>% mutate(n_doses = ifelse(n_doses=="dose_new", "1+", n_doses)) %>% select(-n_doses_new) 
vax_clean2 <- vax_clean2_noadults %>% bind_rows(vax_aNICS_full %>% filter(age_current==25)) %>%  # attach one adult estimate from aNICS for each province (e.g. age 25)
  group_by(pt, location, age_current) %>% filter(value == max(value)) # keep largest estimate if there are duplicates (as aNICS likely underreports?, and for estimates with 1+ and 2 dose we wish to keep the larger)

# export -- vax_clean2 is the cleaned data set for take 2
vax_clean2 %>% readr::write_csv(here::here("data", "measles_vax-coverage-data-clean2.csv"))
vax_clean2_noadults %>% readr::write_csv(here::here("data", "measles_vax-coverage-data-clean2-noadults.csv"))
```
...
vax_terpolated <- vax_dedupe %>%
  ...
  # attach aNICS data where the value is otherwise interpolated
  left_join(vax_aNICS, 
    by = join_by(pt, n_doses, age_current),  ## LH: We may not wish to group by n_doses here. Currently only Alberta aNICS data being appended as all others have n_doses = NA.
    suffix = c("", "_a")
  ) |>
  mutate(
    value = if_else(source == "interpolated" & !is.na(value_a), value_a, value),
